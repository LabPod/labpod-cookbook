...
| [`quantum-science/`](quantum-science/) ([`.tar`](dist/quantum-science.labpod-bundle.tar)) | [pyscf-quantum-chemistry](quantum-science/notebooks/pyscf-quantum-chemistry.ipynb) | Electronic structure with PySCF - H2 bond energy, verified against the textbook value and a bond-length scan that finds the right minimum. |
| same bundle | [qiskit-basics](quantum-science/notebooks/qiskit-basics.ipynb) | Quantum circuit simulation with Qiskit - a Bell state (entanglement) and Grover's search, both verified numerically. |
